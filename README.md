...
- **Volume and output control** -- system volume slider with mute indicator, click-to-rename active speaker name in collapsed or expanded mode, and an expanded list with large radio targets plus enabled/disabled/hidden controls for playback devices. Enabled built-in speakers sort first; hidden devices remain recoverable through a "Show hidden outputs" control.
