...
- **Tile Snap** (macOS + Windows + Linux/X11) -- drag a window to a screen edge to draw drop zones, preview the target on only that display, and snap it into a tiled layout. Enable/disable via the Tile Snap toggle in Settings; Windows defaults it off to avoid conflicting with Windows Snap
