...
source committed into this repo. Display DJ-local modules are marked
`_(local)_` in the provenance table and must not be overwritten during
vendor refreshes.
...
| `src-tauri/src/core/volume.rs`     | `src/main.rs`    | `3b9306d`       | Extracted `get_volume` / `set_volume` / `set_mute` per-OS impls + `VolumeInfo`; Windows volume is locally replaced with native `IAudioEndpointVolume`.                                                                                                                                                    |
| `src-tauri/src/core/audio_output/` | _(local)_        | _n/a_           | Display DJ-local playback-device enumeration, default selection, and stable endpoint IDs (CoreAudio, MMDevice/PolicyConfig, `pactl`).                                                                                                                                                                     |
...
`windows.rs`, `theme.rs`, `wallpaper.rs` — every
