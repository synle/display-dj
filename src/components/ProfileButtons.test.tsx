...
describe('ProfileButtons', () => {
  ...
  it('renders nothing when profiles is empty', () => {
    const { container } = render(<ProfileButtons profiles={[]} onActivate={() => {}} />);
  ...
  it('calls onActivate with correct index when clicked', async () => {
    ...
    render(<ProfileButtons profiles={mockProfiles} onActivate={onActivate} />);
  ...
  it('shows fallback name for unnamed profiles', () => {
    const profiles: Profile[] = [{ name: '', command: 'command/changeVolume/0' }];
  ...
  it('opens overflow menu and activates profile by index', async () => {
    const onActivate = vi.fn();
    const user = userEvent.setup();
    render(<ProfileButtons profiles={[...mockProfiles, extraProfile]} onActivate={onActivate} />);
