...
const _getVolumeHelperBinary = async () =>
  path.join(process['resourcesPath'], `win32_volume_helper.exe`);

const SoundUtils = {
  getVolume: async () => {
    const splits = (await executeBash(await _getVolumeHelperBinary()))
      .split(/[ ]+/)
      .map((s) => s.trim())
      .filter((s) => s);
  ...
  setVolume: async (value: number) =>
    await executeBash(`${await _getVolumeHelperBinary()} ${value}`),
  setMuted: async (muted: boolean) =>
    await executeBash(`${await _getVolumeHelperBinary()} ${muted ? 'mute' : 'unmute'}`),
};
