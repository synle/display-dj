...
- [x] **Auto dark mode schedule** — Toggle dark/light mode on the same night mode schedule that already controls brightness. The 60-second timer loop in `lib.rs` already evaluates time — extend it to also call `core::theme::set_dark_mode` alongside the brightness change. Add a `darkModeAtNight` boolean to `NightModeSchedule` preferences.
- [ ] **Scroll-to-adjust brightness on tray icon** — Mouse wheel on the tray icon nudges all-monitor brightness up/down by a step (e.g., 5%). Tauri's `TrayIconEvent` already exposes scroll events (`TrayIconEvent::Scroll`). Call `core::display::set_all_brightness` on each tick. Gives quick adjustment without opening the popup.
...
- [ ] **Window z-order control (bring to front / send to back)** — Add commands to raise or lower a window in the stacking order. Two scopes × three actions = 6 commands, kept to a single namespace segment for consistency with `command/tile/...` and `command/wallpaper/...`:
  - `command/window/bringToFront` — raise focused window above all others
  - `command/window/sendToBack` — lower focused window behind all others
  - `command/window/toggleFrontBack` — if focused → send to back; if not focused → bring to front
  - `command/app/bringToFront` — raise all windows of the focused app above all others
  - `command/app/sendToBack` — lower all windows of the focused app behind all others
  - `command/app/toggleFrontBack` — same toggle logic but applied to the whole app
    Per-platform feasibility:
  - **macOS**: front via `kAXRaiseAction` + `NSRunningApplication.activate(options:)`; back has **no public AX API** — needs CGS private APIs (`CGSOrderWindow` with `kCGSOrderBelow`), same trust class as the `_AXUIElementGetWindow` and Spaces collapsing we already use. App-scope variant enumerates AXWindows of the AXApplication. Toggle uses `kAXFocusedWindow` for "is focused" and `CGWindowListCopyWindowInfo` (front-to-back order) for "is at back."
  - **Windows**: trivial — `SetWindowPos(hwnd, HWND_TOP, ...)` + `SetForegroundWindow` for front; `SetWindowPos(hwnd, HWND_BOTTOM, ...)` for back. App-scope variant uses `EnumWindows` filtered by owning PID. Toggle uses `GetForegroundWindow` + `EnumWindows` z-order.
  - **Linux/X11**: `_NET_ACTIVE_WINDOW` for front, `_NET_RESTACK_WINDOW`/`XLowerWindow` for back. App-scope filters `_NET_CLIENT_LIST` by `_NET_WM_PID`. Toggle uses `_NET_CLIENT_LIST_STACKING`.
    Implementation lives alongside tiling: extend `tiling/{macos,windows,linux}.rs` (already do focused-window resolution + multi-window enumeration) or add a sibling `zorder.rs`. Wire into `tray.rs` `execute_command()` dispatch so the commands work from keyboard shortcuts, profiles, layout presets, and the tray menu. Open question: whether to use the macOS CGS private API for "send to back" — if no, ship "front" + "toggle (raise-only)" on macOS and document the gap.
- [ ] **Per-monitor brightness in profiles** — Profiles currently set one brightness for all monitors via `command/changeBrightness/{value}`. Extend to support per-monitor targets (e.g., `command/changeMonitorBrightness/{uid}/{value}`). `core::display::set_one_brightness` already supports per-monitor targeting. Useful for mixed setups (dim external, bright built-in).
...
- [ ] **Monitor input switching** — DDC/CI supports input source switching on external monitors (`VCP code 0x60`). Would enable KVM-like workflows: one keyboard shortcut to switch a monitor between laptop and desktop inputs. Add a new method to `core::DisplayControl` (or a new free function in `core::display`) that issues the VCP `0x60` write through the existing `ddc-macos`/`ddc-winapi`/`ddcutil` paths. UI: dropdown per monitor showing available inputs (HDMI1, DP1, USB-C, etc.).
...
- [ ] **Remote control via local web UI** — Expose a lightweight HTTP interface on the local network so users can adjust brightness/volume/dark mode from a phone or tablet. As of v7.0.0 Display DJ no longer runs any HTTP server, so this would mean **standing up a fresh HTTP server inside `lib.rs`** (e.g., axum, or a tiny `std::net::TcpListener` accept loop) whose handlers call into `core::*` directly. Future work, not blocked on any external CLI. Useful for home theater setups or adjusting a docked laptop from across the room.
- [ ] **CLI companion commands** — Allow controlling the running app from the terminal (e.g., `display-dj2 set-brightness 50`, `display-dj2 activate-profile Focus`). Useful for scripting, automation, and integration with tools like Raycast, Alfred, or shell aliases. Two viable implementations: (a) have a small standalone CLI (or the upstream `display-dj-cli`) talk to the running GUI via Tauri's `tauri-plugin-single-instance` — the second-instance callback already fires with the spawned argv, so the GUI can interpret CLI args and route them through `tray::execute_command` / `core::*`; or (b) spin up a small in-process IPC channel (Unix socket / named pipe) inside Display DJ that the CLI wrapper writes commands to.
