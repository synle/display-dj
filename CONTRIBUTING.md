...
- **Tray popup placement**: test horizontal and vertical multi-monitor layouts with mixed scaling. Left-click each display's tray icon; the popup must stay fully inside that same display, below top-edge trays and above bottom-edge trays. Enable Debug Logging and verify both left/right clicks record mouse position, selected screen, all screen bounds/scales, and final popup placement.
...
- **Task View or Show Desktop shortcut does nothing across an elevation boundary** — on Windows, Display DJ sends `Win+Tab` or `Win+D` at its current integrity level. Restart Display DJ as administrator only when Windows blocks injected input across that boundary. On macOS, verify the matching `Ctrl+Cmd+Shift+Up/Down` defaults open Mission Control and toggle the desktop.
