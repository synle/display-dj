...
- **Task View or Show Desktop shortcut does nothing across an elevation boundary** — Display DJ sends `Win+Tab` or `Win+D` at its current integrity level. Restart Display DJ as administrator only when Windows blocks injected input across that boundary.
