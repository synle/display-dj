...
- **Task View or Show Desktop shortcut does nothing across an elevation boundary** — system shortcuts dispatch after key release. On Windows, Display DJ then sends `Win+Tab` or `Win+D` at its current integrity level. Restart Display DJ as administrator only when Windows blocks injected input across that boundary. On macOS, Display DJ sends Mission Control and Show Desktop notifications directly to the Dock; verify the matching `Ctrl+Cmd+Shift+Up/Down` defaults open Mission Control and toggle the desktop.
