...
- **Task View or Show Desktop shortcut does nothing across an elevation boundary** — system shortcuts dispatch after arrow release. On Windows, Task View waits for physical modifier release and sends `Win+Tab` at Display DJ's current integrity level; restart Display DJ as administrator only when Windows blocks injected input across that boundary. Show Desktop uses `Shell.ToggleDesktop()` and injects no keys. On macOS, Display DJ sends Mission Control and Show Desktop notifications directly to the Dock; verify the matching `Ctrl+Cmd+Shift+Up/Down` defaults open Mission Control and toggle the desktop.
