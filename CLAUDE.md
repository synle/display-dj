...
- **Unit tests**: Inline `#[cfg(test)]` modules in `config.rs`, `display.rs`, `keep_awake.rs`, `tray_icon.rs`, and `tiling.rs`
  ...
  - `tiling/mod.rs` (shared types + layout math + orchestration): TilingLayout parsing, layout calculation for all 17 layouts (halves, thirds, two-thirds, quarters, maximize), gap/padding math, custom ratio support, TilingState creation, `layout_across_displays` multi-display overflow with oversized window handling, layout preset resolution (by index and name), window-to-rule matching (substring, case-insensitive, first-match-wins, unknown layout skip), `plan_expose` (all-windows placement), `plan_expose_app` (target app + others on remaining displays, no overflow to consumed displays), `plan_layout_preset` (preset rule matching + placement)
  - `tiling/macos.rs` (macOS only): macOS-specific AXUIElement window manipulation, NSScreen display detection, Tile Snap via NSEvent global monitor, `is_window_move` (move vs resize detection), `build_snap_zones` / `detect_snap_zone_macos` (rectangle hit-test zone detection)
  - `tiling/windows.rs` (Windows only): Win32 window manipulation via GetForegroundWindow/SetWindowPos, EnumDisplayMonitors for display detection, EnumWindows for Expose, `should_skip_system_window` filtering (Program Manager, TextInputHost, etc.), IsZoomed restore, DWM border over-expand (Option B), `dbg_log` helper for debug file logging, expose debounce via AtomicBool
