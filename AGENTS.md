...
- `VolumeControl.tsx` always shows a readonly `Output Speakers (<enabled non-hidden count>)` section label above the volume slider; collapsed mode appends `- <selected name>` inline, expanded mode shows the selected name below the count as an editable blue link. Expanded monitor mode adds one padded radio target per output, click-to-edit row names, and one `enabled` / `disabled` / `hidden` selector. Hidden outputs stay out of the normal list but remain recoverable through "Show hidden outputs".
