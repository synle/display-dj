...
macOS defaults `Ctrl+Cmd+Shift+Up` to `command/system/taskView` (Mission Control) and `Ctrl+Cmd+Shift+Down` to `command/system/showDesktop`; both post Dock distributed notifications. Windows uses matching `Ctrl+Alt+Shift` defaults and releases the triggering modifiers before synthesizing `Win+Tab` or `Win+D` through `SendInput`. Linux does not register these platform-specific defaults.
