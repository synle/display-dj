...
Windows Tile Snap uses `SetWinEventHook(EVENT_SYSTEM_MOVESIZESTART..=EVENT_SYSTEM_MOVESIZEEND)` on a dedicated message-pump thread. `WM_NCHITTEST` rejects resize-border gestures; active title-bar moves poll `GetCursorPos` and render one transparent, click-through Tauri overlay per display. Each overlay listens on a display-scoped event name passed in its page URL, so multi-monitor previews cannot overwrite or duplicate each other. Maximized windows must run through `restore_maximized_window()` before drag bounds are captured so title-bar dragging can unsnap them. Lazy WebView creation must dispatch through `run_on_main_thread`; Win32 polling remains on the monitor thread. On move end, the exact rectangle shown in the preview is applied with `SetWindowPos`.
