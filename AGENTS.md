...
- `VolumeControl.tsx` has its own expand/collapse chevron independent from the monitor section. Its readonly `All Speakers (<enabled non-hidden count>)` label appends `- <selected name>` while collapsed; expanded mode lists every output, including the selected endpoint with its radio checked, using monitor-name font sizing, one padded radio target, click-to-edit name, and an `enabled` / `disabled` / `hidden` selector per row. Hidden outputs stay out of the normal list but remain recoverable through "Show hidden outputs".
